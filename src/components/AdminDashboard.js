...
function AdminDashboard(){
    ...
    const [newWorkshop, setNewWorkshop] = useState({ email: '', address: '', phoneNumber: '' });
    ...
    useEffect(() =>{
        ...
        const fetchWorkshops = async() => {
            try{
                const response = await axios.get('http://localhost:5109/api/admin/workshops', {
                    headers: { Authorization: `Bearer ${token}`}
                });
    ...
    const handleAddWorkshop = async (e) => {
        ...
        try {
            const addWorkshopResponse = await axios.post(`http://localhost:5109/api/admin/workshop`, newWorkshopData, {
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            <FormContainer>
                    ...
                    <form onSubmit={handleAddWorkshop}>
                        ...
                        <FormInput
                            ...
                            onChange={(e) => setNewWorkshop({ ...newWorkshop, phoneNumber: e.target.value })}
                            required
                        />
